...
class DoIPSession:
    ...
    async def _handle_entity_status(self) -> None:
        ...
        Response payload (ISO 13400-2):
          ...
          byte 2: currently open sockets (Dynamic count)
          bytes 3-6: max data size (big-endian uint32)
        """
        # Dynamically determine the number of open/activated sockets
        # We query the registry to see how many sessions are currently active.
        # Note: In this implementation, sessions are registered ONLY upon 
        # successful Routing Activation (see _handle_routing_activation).
        if self._registry is not None:
            try:
                # Assuming SessionRegistry behaves like a dict or list
                currently_open = len(self._registry)
            except Exception:
                # Fallback to checking current session state if registry counting fails
                currently_open = 1 if self.activated else 0
        else:
            # Fallback if no registry is provided
            currently_open = 1 if self.activated else 0

        payload = bytes([
            self.config.doip.node_type,
            0x01,  # max sockets (PoC)
            currently_open,  # Dynamic value
